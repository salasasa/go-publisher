...
func PostNews(uid int, title, content string) (int, error) {
	...
	if err := GoPublisherDB.Create(&news).Error; err != nil {
...
func DeleteNews(id int) error {
	tx := GoPublisherDB.Model(&model.News{}).Where("id=? and delete_time is null", id).Update("delete_time", time.Now())
	if tx.Error != nil {
...
func UpdateNews(id int, title, content string) error {
	tx := GoPublisherDB.Model(&model.News{}).Where("id=? and delete_time is null", id).Updates(map[string]any{"title": title, "article": content})
	if tx.Error != nil {
...
func GetNewsById(id int) *model.News {
	...
	tx := GoPublisherDB.Select("*").Where("delete_time is null").First(news) //隐含的where条件是id。注意：Find不会返回ErrRecordNotFound
	if tx.Error != nil {
...
func GetNewsByUid(uid int) []*model.News {
	var news []*model.News
	tx := GoPublisherDB.Select("*").Where("delete_time is null and user_id=?", uid).Find(&news)
	if tx.Error != nil {
...
func GetNewsByPage(pageNo, pageSize int) (int, []*model.News) {
	var total int64
	err := GoPublisherDB.Model(model.News{}).Where("delete_time is null").Count(&total).Error
	if err != nil {
	...
	var news []*model.News
	// 新闻按发布时间降序排列
	tx := GoPublisherDB.Select("*").Where("delete_time is null").Order("create_time desc").Limit(pageSize).Offset(pageSize * (pageNo - 1)).Find(&news)
	if tx.Error != nil {
