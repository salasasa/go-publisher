...
func PostNews(ctx *gin.Context) {
	...
	id, err := gorm.PostNews(loginUid, news.Title, news.Content)
	if err != nil {
...
func GetNewsById(ctx *gin.Context) {
	...
	if id, err := strconv.Atoi(idStr); err != nil || id <= 0 {
		...
		news := gorm.GetNewsById(id)
		if news == nil {
			ctx.Status(http.StatusNotFound)
			return
		}

		user := gorm.GetUserById(news.UserId)
		if user != nil {
...
func UpdateNews(ctx *gin.Context) {
	...
	err = gorm.UpdateNews(news.Id, news.Title, news.Content)
	if err != nil {
		ctx.String(http.StatusInternalServerError, err.Error())
		return
	}
...
func NewsList(ctx *gin.Context) {
	...
	total, news := gorm.GetNewsByPage(PageNo, PageSize)
	if total < len(news) {
		ctx.String(http.StatusInternalServerError, "出错了")
		return
	}
	for _, ele := range news {
		user := gorm.GetUserById(ele.UserId)
		if user != nil {
